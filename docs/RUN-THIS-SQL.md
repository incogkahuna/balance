...
>
> **If you already ran an earlier version of this block**, also run the
> small "lock to 3" snippet at the end of this section to remove the
> Brian/Mark pre-grants and the old admin fallback.
...
## ⭑ NEW 2026-07-21 — LED WALLS TABLE (gear off localStorage) — RUN THIS TOO

> **STATUS: ⬜ NOT YET RUN.** One paste, idempotent. Moves the Gear database
> off per-browser localStorage so every wall picker (production form, intake,
> gear page) shares one live list — the fix for "wall dropdown not pulling
> from gear". Until it runs, gear keeps working per-browser as before; the
> first signed-in device after the migration auto-imports its local walls.
