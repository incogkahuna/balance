...
create table if not exists public.pipeline_deals (
  ...
  -- Deal documents: [{id, name, category, bucket, path, at, by}] — COI,
  -- executed agreement, invoices, completion files (2026-07-24).
...
-- Idempotent upgrade for DBs created before the files column existed.
...
create table if not exists public.pipeline_quotes (
  ...
  discount           jsonb,
  -- Free-form "custom deal" items outside the rate card:
  -- [{id, name, rate, qty, unit}] (2026-07-24).
...
-- Idempotent upgrade for DBs created before custom_lines existed.
